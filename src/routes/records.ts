...
router.get('/', requireRoles(['ADMIN', 'ANALYST']), async (req, res) => {
  try {
    const { type, category, startDate, endDate, skip = 0, take = 50 } = req.query;
...
router.get('/:id', requireRoles(['ADMIN', 'ANALYST']), async (req, res) => {
  try {
    const record = await prisma.record.findUnique({ where: { id: req.params.id as string } });
...
router.delete('/:id', requireRoles(['ADMIN']), async (req, res) => {
  try {
    await prisma.record.delete({ where: { id: req.params.id as string } });
